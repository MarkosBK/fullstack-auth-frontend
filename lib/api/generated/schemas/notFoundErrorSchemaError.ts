/**
 * Generated by orval v7.13.0 🍺
 * Do not edit manually.
 * Fullstack Auth API
 * Complete authentication and user management API for web and mobile applications. Supports JWT authentication, role-based access control, and user management with caching.
 * OpenAPI spec version: 1.0
 */
import type { NotFoundErrorSchemaErrorMessage } from './notFoundErrorSchemaErrorMessage';

export type NotFoundErrorSchemaError = {
  message?: NotFoundErrorSchemaErrorMessage;
  timestamp?: string;
  path?: string;
  method?: string;
};
